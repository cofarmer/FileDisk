...
)
{
    ...
    {
		fprintf(stderr, "DeviceIoControl IOCTL_FILE_DISK_OPEN_FILE failed, error code = %d \n", GetLastError());
	...
	wsprintfA("%s\\", &VolumeName[4]);

	SHChangeNotify(SHCNE_DRIVEADD, SHCNF_PATHA, mount_point, NULL);
	...
	dev_broadcast_volume.dbcv_unitmask = 1 << (mount_point[0] - L'A');
...
int FileDiskUmount(char DriveLetter)
{
    ...
    {
		fprintf(stderr, "DeviceIoControl FSCTL_LOCK_VOLUME failed, error code = %d \n", GetLastError());
        PrintLastError(&VolumeName[4]);
        return -1;
    }
    ...
    {
		fprintf(stderr, "DeviceIoControl IOCTL_FILE_DISK_CLOSE_FILE failed, error code = %d \n", GetLastError());
        PrintLastError("FileDisk:");
        return -1;
    }
    ...
    {
		fprintf(stderr, "DeviceIoControl FSCTL_DISMOUNT_VOLUME failed, error code = %d \n", GetLastError());
        PrintLastError(&VolumeName[4]);
        return -1;
    }
    ...
    {
		fprintf(stderr, "DeviceIoControl FSCTL_UNLOCK_VOLUME failed, error code = %d \n", GetLastError());
        PrintLastError(&VolumeName[4]);
        return -1;
    }
    ...
    {
		fprintf(stderr, "DefineDosDevice DDD_REMOVE_DEFINITION failed, error code = %d \n", GetLastError());
        PrintLastError(&VolumeName[4]);
        return -1;
    }
...
int __cdecl main(int argc, char* argv[])
{
    ...
    if ((argc == 5 || argc == 6) && !strcmp(Command, "/mount"))
    {
        ...
        if (FileName[0] == '\\')
        {
            ...
            {
                strcpy((char *)OpenFileInformation->FileName, "\\??\\UNC");
                strcat((char *)OpenFileInformation->FileName, FileName + 1);
            }
            else
                // \Device\Harddisk0\Partition1\path\filedisk.img
            {
                strcpy((char *)OpenFileInformation->FileName, FileName);
            }
        }
        else
            // c:\path\filedisk.img
